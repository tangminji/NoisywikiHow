...
class pNorm(nn.Module):
    def __init__(self, p=0.5, reduction='none'):
...
class SRLoss(nn.Module):
    def __init__(self, criterion, lamb=None, tau=None, p=None, reduction='none'):
        super(SRLoss, self).__init__()
        self.criterion = criterion
        self.norm = pNorm(p=p, reduction=reduction)
        self.lamb = lamb
        self.tau = tau
    ...
    def forward(self, pred, labels):
        ...
        out = out/self.tau
        loss = self.criterion(out, labels) + self.lamb * self.norm(out)
        return loss
    # adjust lambda
    def update_lamb(self, rho):
        self.lamb = self.lamb*rho
...
class CELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        if len(labels.size()) == 1:
            ...
            loss = -torch.sum(F.log_softmax(pred, dim=1) * labels, dim=1)
            if self.reduction == 'none':
                return loss
            elif self.reduction == 'mean':
                return loss.mean()
...
class MAELoss(nn.Module):
    def __init__(self, num_classes=10, scale=2.0, reduction='none'):
    ...
    def forward(self, pred, labels):
        ...
        loss = 1. - torch.sum(labels * pred, dim=1)
        if self.reduction == 'none':
...
class GCELoss(nn.Module):
    def __init__(self, num_classes=10, q=0.7, reduction='none'):
    ...
    def forward(self, pred, labels):
        ...
        if len(labels.size()) == 1:
            label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
            labels = label_one_hot
        loss = (1. - torch.pow(torch.sum(labels * pred, dim=1), self.q)) / self.q
        if self.reduction == 'none':
            return loss
        elif self.reduction == 'mean':
            return loss.mean()
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=10, a=1, q=2, eps=eps, scale=1., reduction='mean'):
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=10, a=3, q=1.5, reduction='none'):
        super(NCEandAGCE, self).__init__()
        self.num_classes = num_classes
        self.nce = NCELoss(num_classes=num_classes, scale=alpha, reduction=reduction)
        self.agce = AGCELoss(num_classes=num_classes, a=a, q=q, scale=beta, reduction=reduction)
